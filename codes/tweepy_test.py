...
results = api.search("Apple",count=100, lang = "en") #trying out "en" first in this demo
...
for tweet in results:
    ...
    #we want to exclude retweets and quoted tweets in our corpus
    if not tweet._json['is_quote_status'] and not tweet._json['retweeted']:
        ...
        #could be useful for annotating user's identity with the tech company he tweeted
...
with open('twitter_apple_scrap_string.csv', 'a', newline='', encoding='utf-8') as csvfile:
    fieldnames = ['created_at', 'tweet_id','text','lang','user_info']
